...
/**
 * Accion que agrega un ticket hijo a un incidente
 * @version 1.0.0
 */
public class DerivedAction extends ActionSupport implements SessionAware {

  
  /** Auxiliar de serializacion */
  private static final long serialVersionUID = 2405268300498412246L;
  ...
  public String addDerived() throws Exception {

    String loginName = (String) session.get( IGlobalConstants.SESS_LOGIN_NAME );
    if ( loginName == null || loginName.isEmpty()) {
       session.put( IGlobalConstants.SESS_TARGET_URL, IGlobalConstants.PAGE_DERIVED_FORM );
       return "notValid";
    }
    HttpServletRequest  req  = ServletActionContext.getRequest();

    String bufferID  = req.getParameter( IGlobalConstants.SESS_PARENT_ID );
    String bufferPID = this.parentID;
    if (( bufferID == null || bufferID.isEmpty()) && ( bufferPID == null || bufferPID.isEmpty())) {
       return "notValid";
    }
    if ( bufferID != null && !bufferID.isEmpty())
       this.parentID = TicketUtils.uncloackData( bufferID );
    else 
       this.parentID = bufferPID;

    CIncidentService svcParent  = new CIncidentService();    
    CriticalIncident parentBean = svcParent.getIncident( Integer.parseInt( parentID ));
    String           impactName = this.getImpactName( parentBean.getImpact());
    
    if (( title == null || title.isEmpty()) || ( notes == null || notes.isEmpty())) {
       
       session.put( IGlobalConstants.SESS_PARENT_ID,   parentID );
       session.put( IGlobalConstants.SESS_REMEDY_ID,   parentBean.getRemedyID());
       session.put( IGlobalConstants.SESS_IMPACT_NAME, impactName );
       session.put( IGlobalConstants.SESS_PAR_BEAN,    parentBean );
    ...
    bean.setImpact( impact );
    bean.setUrgency( urgency );
    bean.setServiceType( serviceType );
    bean.setAssignedGroup( assignedGroup );
    bean.setParentID( remedyID );
  ...
  public String getImpactName( String impactID ) { 
    
    IGlobalService gsvc = new GlobalService();
    Map<String,String> impactMap = gsvc.getImpactData();
    String result = impactMap.get( impactID );
    if ( result == null || result.isEmpty()) { 
       
       ParametersService svcParams = new ParametersService();
       result = svcParams.getParams().getIncidentDefaults().getImpact();
    }
    return result;
  }
